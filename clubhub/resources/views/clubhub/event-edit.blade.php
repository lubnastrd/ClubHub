...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-red-600 to-red-700 rounded-lg shadow-lg p-6 text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold mb-2 text-white">Edit Event</h1>
                    <p class="text-white">Edit event atau turnamen futsal</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('event.update', $event->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700 mb-2">Nama Event *</label>
                <input type="text" id="nama" name="nama" value="{{ old('nama', $event->nama) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-900" style="color: #111827;">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="tanggal_mulai" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Mulai *</label>
                    <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', $event->tanggal_mulai->format('Y-m-d')) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-900 cursor-pointer" style="color: #111827;" placeholder="Klik untuk memilih tanggal">
                </div>
                <div>
                    <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Selesai</label>
                    <input type="date" id="tanggal_selesai" name="tanggal_selesai" value="{{ old('tanggal_selesai', $event->tanggal_selesai?->format('Y-m-d')) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-900 cursor-pointer" style="color: #111827;" placeholder="Klik untuk memilih tanggal">
                </div>
            </div>

            <div>
                <label for="lokasi" class="block text-sm font-medium text-gray-700 mb-2">Lokasi *</label>
                <input type="text" id="lokasi" name="lokasi" value="{{ old('lokasi', $event->lokasi) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-900" style="color: #111827;">
            </div>

            <div>
                <label for="deskripsi" class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                <textarea id="deskripsi" name="deskripsi" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-900" style="color: #111827;">{{ old('deskripsi', $event->deskripsi) }}</textarea>
            </div>
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <img src="{{ asset('storage/' . $event->poster) }}" alt="Poster Event" class="max-w-md rounded-lg shadow-md">
                </div>
                @endif
                ...
                <p class="mt-1 text-sm text-gray-500">Format: JPEG, PNG, JPG, GIF. Maksimal 2MB. Kosongkan jika tidak ingin mengubah poster.</p>
